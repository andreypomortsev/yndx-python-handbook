x, y, flag = float(input()), float(input()), True
...
if (-7 <= x <= 5) and y <= 0:
    # 0.2 коэффициент из f(x) = (1/4)(x + 7)(x - 5)
    # -1 x-координат вертекса
    # -9 y-координат вертекса
    parabola_y = 0.2 * (x - (-1)) ** 2 + (-9)
    if y >= parabola_y:
        flag = False
...
if (-7 <= x <= 0) and (0 <= y <= 5):
    ...
    # Используем формулу b = y - m * x, где (x2, y2) - точка на линии, а m - коэффициент наклона
    intercept = 5 - slope * (-4)
