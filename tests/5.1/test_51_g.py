...
from tests.data.test_data_51 import (rectangle_area, rectangle_move,
                                     ...
                                     rectangle_scale, rectangle_turn)
...
@pytest.mark.parametrize(
    "points, positions, sizes, repeats, _",
    rectangle_turn,
    ids=[i[-1] for i in rectangle_turn],
)
def test_rectangle_turn_test(
    load_module: Callable[[str], ModuleType],
    request: pytest.FixtureRequest,
    points: Tuple[
        Tuple[float | int, float | int], Tuple[float | int, float | int]
...
@pytest.mark.parametrize(
    "points, positions, sizes, factor, _",
    rectangle_scale,
    ids=[i[-1] for i in rectangle_scale],
)
def test_rectangle_scale_test(
    load_module: Callable[[str], ModuleType],
    request: pytest.FixtureRequest,
    points: Tuple[
        Tuple[float | int, float | int], Tuple[float | int, float | int]
